...
- **Skills**: `~/.omp/skills/<name>` — all seven, including `plan-fable-execute-codex`.
- **Agents**: `~/.omp/agent/agents/*.md` — all six (`fable-architect`, `codex-scout`, `codex-executor`, `sonnet-reviewer`, `opus-redteam`, `codex-reviewer`) with pinned models, tool allowlists, and `spawns` permissions. These use OMP-specific frontmatter and features (`irc`, `agent://`, model pins) and are not portable.
...
- **Config levers** (in `~/.omp/agent/config.yml`, not this repo):
  ...
  - `modelRoles.slow` → `claude-opus-4-8:high`; `modelRoles.plan` → `claude-fable-5:high`.
  - Compaction defaults are research-aligned (compact ~85%, keep 20K recent); leave them.
- **Verify**: `omp --list-models` shows the pinned models authed; spawning `fable-architect` from a session resolves; `readlink ~/.omp/agent/agents/fable-architect.md` points into `~/.agents/agents/`.

## Claude Code

- **Skills**: `~/.claude/skills/<name>` — all seven discoverable.
  - Caveat: `plan-fable-execute-codex` references OMP agents that do not exist in Claude Code. Its triggers are explicit phrases, so it stays dormant; if invoked, treat it as OMP-only and fall back to plan-then-implement in one session.
...
- The GPT-5.5 side of the multi-model workflow (`codex-scout`, `codex-executor`, `codex-reviewer`) runs through OMP's `openai-codex` provider — the Codex CLI itself is not involved.
