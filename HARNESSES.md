...
- **Skills**: `~/.omp/skills/<name>` — all eleven, including `plan-opus-execute-codex`, `plan-opus-execute-sonnet`, `plan-codex-execute-opus`, `plan-fable-execute-codex`, and `fleet`.
- **Agents**: `~/.omp/agent/agents/*.md` — all eleven (`opus-architect`, `codex-architect`, `fable-principal`, `sonnet-scout`, `codex-executor`, `opus-executor`, `sonnet-executor`, `opus-reviewer`, `codex-reviewer`, `codex-redteam`, `opus-redteam`) with pinned models, tool allowlists, and `spawns` permissions. OMP-specific (`irc`, `agent://`, model pins); not portable.
...
- **Config levers** (`~/.omp/agent/config.yml`, not this repo):
  ...
  - `modelRoles.slow` → `claude-opus-4-8:high`; `modelRoles.plan` → `claude-opus-4-8:high` (the workflows ignore this role and pin models per agent — Fable runs only as the pinned `fable-principal` decision seat).
...
- **Skills**: `~/.claude/skills/<name>` — all eleven discoverable. Caveat: `plan-opus-execute-codex`, `plan-opus-execute-sonnet`, `plan-codex-execute-opus`, `plan-fable-execute-codex`, and `fleet` reference OMP-only agents; their triggers are explicit phrases so they stay dormant — if invoked, fall back to plan-then-implement (or sequential execution) in one session.
