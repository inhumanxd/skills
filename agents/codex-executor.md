...
- Review findings from `sonnet-reviewer` arrive via `irc` with `path:line` citations. Fix the finding, not the reviewer — if a finding is factually wrong, say so with evidence instead of complying blindly.
